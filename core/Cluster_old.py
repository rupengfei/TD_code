...
def meshRepairWin(width, high):
    ...
    cmds.button(w=width / 3, h=high / 4, label='|<', bgc=[.588, .268, .341],
                command=("setKeyFrameCmd(1,'-');mm.eval('playButtonStepBackward')"))
    cmds.button(w=width / 3, h=high / 4, label='>|', bgc=[.588, .268, .341],
                command=("setKeyFrameCmd(1,'+');mm.eval('playButtonStepForward')"))
    cmds.setParent('..')
...
def spineJoint_str(rootJoint, bCurve):
    ...
    for c in range(mdnum):
        ...
        if mdnum > c > 0:
            cmds.connectAttr((nodes['GlobalScale' + str(c - 1)] + '.outputX'),
                             (nodes['GlobalScale' + str(c)] + '.input1X'))
        if c == mdnum - 1:
...
def dynaimicsAttr2():
    attr = [['fkCtrlVis', 'long', '0,1,1', ''], ['ikCtrlVis', 'long', '0,1,1', ''],
            ['localCtrlVis', 'long', '0,1,0', ''], ['dynamicVis', 'long', '0,1,0', '']]
    return (attr)


def dynaimicsAttr1():
    attr = [['simulationMethod', 'enum', 'Off,Static,DynamicFolliclesOnly,AllFollicles', ''],
            ['collide', 'double', '0,1,1', ''], ['startFrame', 'long', ',,1', ''],
            ['stiffness', 'double', '0,1,.75', ''], ['drag', 'double', ',,0.1', ''],
            ['motionDrag', 'double', ',,0', ''], ['damp', 'double', '0,1,0', ''], ['friction', 'double', '0,,1', ''],
            ['mass', 'double', '0,,5', ''], ['gravity', 'double', ',,9.8', ''],
            ['dynamicsWeight', 'double', '0,1,1', ''], ['startCurveAttract', 'double', '0,1,.85', ''],
            ['attractionDamp', 'double', '0,1,0', ''], ['attractionScale_Position', 'double', '0,1,0', '']]
    return attr
...
def createRollIK(listJnts, groups, intensionYAttr, offsetYAttr, intensionZAttr, offsetZAttr):
    ...
    node = {}
    attr = [[intensionYAttr, 'double', '', ''], [offsetYAttr, 'double', ',,1', ''], [intensionZAttr, 'double', '', ''],
            [offsetZAttr, 'double', ',,1', '']]
    for i in range(len(attr)):
...
def meshRepairCmd():
    ...
    if size > 0 and '.' in listObj[0]:
        ...
        lockAttrList = [nodes['G1'], nodes['G2'], nodes['follicles'], nodes['clusterHandle'], nodes['defGrp'],
                        nodes['mdNode1']]
        # lock and hide attr ....
...
def paintClusterWightCmd():
    ...
    if len(listObjs):
        ...
        try:
            ...
            evalCmd = "artAttrToolScript 3 \"cluster\";artSetToolAndSelectAttr( \"artAttrCtx\", \"cluster.%s.weights\" )" % (
                clusters)
            mm.eval(evalCmd)
